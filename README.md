...
- **Confirmaciones** con un diálogo propio (`confirmDialog()`, bottom sheet con foco, Escape y `role="alertdialog"`) antes de eliminar una semana, quitar un ejercicio de la librería o importar datos. La nota de un ejercicio todavía usa el `prompt()` del navegador.
