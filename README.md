...
│   ├── catalog.php                GET catálogo de ejercicios (búsqueda en/es, detalle) — ADR 0021
...
- **Navegación**: barra inferior de 4 destinos (Semana, Historial, Progreso, Calendario) — lo que se usa a diario. **Perfil** (logo/título, izquierda) y **Ajustes** (engranaje, derecha) viven en el header — lo ocasional, ver ADR 0013. El logo es un `<a href="#/perfil">`: en una app de un solo usuario, "la app" y "tu cuenta" son casi lo mismo, como el avatar de otras apps. Cada vista tiene URL por hash (`#/hoy`, `#/historial`, `#/progreso`, `#/calendario`, `#/perfil`, `#/ajustes`; Ajustes además por pestaña: `#/ajustes/split`, `/reglas`, `/ejercicios`, ver ADR 0020) — id/ruta internos de "Semana" siguen siendo `hoy`, ver ADR 0015: el botón atrás recorre las vistas visitadas, un enlace directo abre esa vista y recargar te deja en la misma.
- **Racha**: días "cumplidos" consecutivos, en días (ejercicios marcados ≥ una regla editable en Ajustes, 3 por defecto). El corte ya no es día por día: una semana (lun–sáb, domingo nunca cuenta) necesita **al menos N días cumplidos** (otra regla editable, 5 por defecto) para no romper la racha — si los alcanza, todos sus días cumplidos suman normal; si no, la racha se corta ahí aunque algún día suelto sí haya llegado al mínimo. La semana en curso nunca se juzga como "rota" hasta que termine. Es la card principal de Semana → Resumen (ADR 0022): número grande, medalla (bronce/plata/oro a 10/30/100 días — la más alta ya ganada), mejor racha, barra hacia la siguiente medalla (medida desde cero: días actuales / días de esa medalla) y, debajo, cuántos días faltan esta semana para que la racha siga activa.
- **Semana tiene 2 pestañas** (`.seg-tabs`; ver ADR 0012, ADR 0014 y ADR 0015): **Resumen** (la que abre por default — selector de semana, card de racha, riel de días, siguiente entrenamiento, "Esta semana", nota de la semana) y **Hoy** (selector de día compacto, sesión, panel del día — lo que se toca mientras se entrena). Tocar un día puntual del riel de Resumen manda a Hoy con ese día cargado; elegir una semana se queda en Resumen.
- **Selector de día compacto** (pestaña Hoy): flechas ‹ › + fecha, en vez de un riel de 7 tabs (que vive en Resumen: los 7 días a la vista con su fecha y su estado — cumplido, hoy, siguiente, sin registrar; uno que recibió contenido migrado muestra un pequeño ícono). Cambiar de día también funciona deslizando el panel a los lados (mismo movimiento, `stepActiveDay()`).
- **Panel del día** (pestaña Hoy):
  - Cabecera fusionada: grupo muscular, "N series · N kg" del día, anillo de progreso (ejercicios marcados/total) y botón "Compartir día".
  - Ejercicios editables inline: nombre (con autocompletado contra la librería), kg, reps, series — 7 controles siempre visibles por fila (check, nombre+nota, kg, rep, ser, chevron, asa de arrastre). Al tocar kg/rep/ser para editar, el cursor va directo al final del valor (no donde cayó el toque) — pensado para agregar/corregir el último dígito sin tener que moverlo primero.
  - Nota libre por ejercicio (ej. "incluye barra", "×2 la mancuerna").
  - Chevron expandible: "Ver progreso", "Ver info" (ojo, si hay coincidencia con el dataset) y "Eliminar" (papelera), más la comparación kg/reps/series contra la semana anterior (flechas subir/bajar/igual) y una línea de **progresión sugerida** (kg de la semana pasada + 2.5 kg fijo) cuando esa semana quedó marcada como hecha y su kg es numérico — solo referencia visual, nunca precarga el input.
  - Agregar y eliminar ejercicios (con confirmación al borrar, ahora dentro del detalle del chevron).
  ...
  - Card de sesión (arriba de la tabla de ejercicios): el día de hoy, un botón a todo el ancho "Empezar actividad" con cronómetro en vivo dentro ("Detener actividad"); al detener quedan hora de inicio/fin (HH:MM, editables) y duración con segundos. La hora de inicio se guarda con segundos, así el contador sigue bien aunque recargues o bloquees el celular. Otros días, o "Registrar a mano", muestran los campos directo; borrar la hora de fin reanuda el contador.
  - Conversor kg ⇄ lbs: colapsado detrás de un ícono junto al selector de día — se abre como sheet, no siempre visible. El campo kg recibe el foco de una vez al abrir (con su valor seleccionado), así en celular el teclado numérico aparece sin necesitar un toque extra.
- Pestaña Resumen (ADR 0022): **selector de semana** (abre un sheet con todas las semanas y sus días cumplidos, y "Eliminar esta semana" con doble confirmación) con "+ Nueva", **card de racha** (número, medalla, barra a la siguiente y días que faltan esta semana), **riel de días**, **siguiente entrenamiento** (qué toca, primeros ejercicios, duración estimada, "Empezar" en el de hoy), **Esta semana** (tiempo, kg y series contra la semana pasada), **series por músculo** (contra la meta de 10–20 por semana, cada ejercicio cuenta para su músculo principal; ADR 0023) y **nota libre de la semana** (textarea, se guarda sola al salir del campo).
  - Botón "Compartir resumen semanal" (ícono, junto al selector de semana): clona el bloque de Resumen (header, semana, card de racha, riel de días y "Esta semana") y lo copia al portapapeles como PNG, sin importar qué pestaña (Hoy/Resumen) esté activa en ese momento. "Patrón Strava": una sola card redondeada al 50% de opacidad (no transparente del todo, tampoco sólida) en vez de fondo completamente transparente — pensada para pegarse sobre cualquier fondo al compartir en redes (una foto, no solo un chat oscuro). El título, el subtítulo y la semana van en blanco; la card de racha y los días tienen fondo propio y se leen igual que en la app. El riel de días cabe entero, sin recortes. Si el navegador no soporta copiar imágenes, cae a descarga normal.
...
- **Exportar / importar datos** (Perfil): exportar arma un JSON con todas las semanas/ejercicios ya cargados en memoria (sin pedir nada nuevo a la API) y lo descarga. Importar lee un archivo con esa misma forma y lo manda a `api/import.php`: valida todo antes de escribir (todo o nada, transacción), y por cada semana del archivo — si ya existe en tu base (mismo lunes), reemplaza sus ejercicios por completo; si no existe, la crea. Las semanas que no vienen en el archivo quedan intactas. Antes de enviar, un diálogo (`confirmDialog()`) te dice cuántas semanas se van a reemplazar y cuántas son nuevas.
- **Backups automáticos** (Perfil): lista los volcados que genera el cron del servidor (`api/db/backup_export.php`, ver "Backup automático (cron)" en Despliegue) vía `api/backups.php`, con fecha y tamaño, cada uno descargable con un click — antes solo se podían bajar por FTP.
...
`CACHE_NAME` de `sw.js` se recalcula solo: un git hook (`.githooks/pre-commit` → `scripts/bump-sw-cache.php`) hashea el contenido de `index.html`, todo `css/` (tokens, components, base y views/), `js/changelog-data.js`, `js/app.js`/`js/api.js`/`js/offline-queue.js`/`js/snapshot.js`, `manifest.json` y los íconos en cada commit y reescribe `CACHE_NAME` (`bitacora-shell-v<versión>-<hash10>`, ej. `bitacora-shell-v1.55.3-f517deaf55`) solo si el hash cambió — ya no hace falta acordarse de bumpearlo a mano, que era la causa de que varias veces durante el desarrollo local de este proyecto un navegador con la PWA instalada siguiera sirviendo el shell viejo desde caché. La versión semver (leída de la cabecera más reciente de `CHANGELOG.md`) va en el nombre solo para que se lea en DevTools → Application → Cache Storage y coincida con lo que muestra Perfil → Changelog — lo que de verdad decide si el caché cambia sigue siendo el hash, no la versión. **Activar el hook una sola vez por clon del repo**: `git config core.hooksPath .githooks`.
